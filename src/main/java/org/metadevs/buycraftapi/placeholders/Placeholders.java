...
import static org.metadevs.buycraftapi.data.Type.GLOBAL;
...
public class Placeholders {
	...
	@Setter private int maxPayments;
    ...
    public String onPlaceholderRequest(Player player, @NotNull String identifier) {
        if (identifier.equalsIgnoreCase("value_from_name")) {
            ...
            String value = query.getPlayerTotal(player.getName());
        ...
        if (identifier.contains("recent_name_")) {
	        ...
	        if (num > maxPayments - 1 || num < 0) return "0";
	        //return "Invalid payment number. Put a number from 0 to " + (maxPayments - 1);
        ...
        if (identifier.contains("top_donor_monthly_name_")) {
	        ...
	        return payment == null ? "0" : payment.getName();
        }
        ...
        if (identifier.equalsIgnoreCase("total_earnings_global")) {
	        ...
	        return data == -1 ? "Error" : String.format("%.2f", data);
        }
    ...
    private String getVaultTop(String replace, Type currentMonth) {
	    ...
	    return payment == null ? "0" : buyCraftAPI.getPerms().getPrimaryGroup(null, Bukkit.getOfflinePlayer(payment.getUuid()));
    }
    ...
    private String getPrice(String replace, Type monthly) {
	    ...
	    return payment == null ? "Error" : String.format("%.2f", payment.getAmount());
    }
}
